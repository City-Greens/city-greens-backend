...
app.post("/vender", (req, res) => {
  ...
  if (!name || !address || !email || !phone_number) {
    return res
      ...
      .json({
        error:
          "Please provide name, image, address, description, email, and phone_number",
      });
  }
  db.run(
    "INSERT INTO vender (name, image, address, description, email, phone_number) VALUES (?, ?, ?, ?, ?, ?)",
    [name, image, address, description, email, phone_number],
    function (err) {
      if (err) {
...
app.get("/vender", (req, res) => {
  db.all("SELECT * FROM vender", [], (err, vendors) => {
    ...
    db.all(
      "SELECT * FROM products WHERE venderID IN (" + vendorIds.join(",") + ")",
      [],
      (err, products) => {
        if (err) {
          return res.status(500).json({ error: err.message });
        ...
        const vendorsWithProducts = vendors.map((vendor) => {
          return {
            ...
            products: products.filter(
              (product) => product.venderID === vendor.id,
            ),
          };
        });
...
app.get("/vender/:id", (req, res) => {
  ...
  db.get("SELECT * FROM vender WHERE id = ?", [id], (err, vendor) => {
    ...
    db.all(
      "SELECT * FROM products WHERE venderID = ?",
      [id],
      (err, products) => {
        if (err) {
          return res.status(500).json({ error: err.message });
...
app.put("/vender/:id", (req, res) => {
  ...
  const { name, image, address, description, email, phone_number } = req.body;
  db.run(
    "UPDATE vender SET name = ?, image = ?, address = ?, description = ?, email = ?, phone_number = ? WHERE id = ?",
    [name, image, address, description, email, phone_number, id],
    function (err) {
      if (err) {
        return res.status(500).json({ error: err.message });
      }
